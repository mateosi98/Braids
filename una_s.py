...
def initialize(epoc, train, strands, length, depth):
    x_train, y_train, x_test, y_test, x_all = get_braids_labels(strands, length, depth)
    ...
    return my_lara, x_test_copy, x_all
...
def proceed_una(my_lara, x_test, x_all, test_denom, strands, length):
    ...
    for iteration in range(int(len(x_test)/test_denom)):
        ...
        while not done:
            ...
            if braid in x_all:    
                ...
                action = np.argmax(legal_probabilities)
            else:
            ...
            step += 1
        env.close()
        print('UNA_S,\tProgress:\t{:.3f},\tAccuracy:\t{:.3f}'.format(iteration/int(len(x_test)/test_denom),wins/(iteration+1)))
